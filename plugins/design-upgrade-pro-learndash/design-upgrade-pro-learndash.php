 ...
 */
define( 'LDX_DESIGN_UPGRADE_PRO_LEARNDASH_VERSION', '2.28' );
...
function ldx_design_upgrade_pro_learndash_enqueue_css() {
	...
	if( wp_style_is( 'ldx-design-upgrade-learndash', 'enqueued' ) ) {
		...
		if ( 'legacy' === $template ) {
			...
			wp_enqueue_style( 'ldx-design-upgrade-learndash', plugins_url( 'assets/css/learndash.css', __FILE__ ), array( 'learndash_style', 'sfwd_front_css', 'learndash_template_style_css', 'learndash_quiz_front_css' ), '2.28' );

		} else {

			// Add stylesheet for "LearnDash 3.0" template
			wp_enqueue_style( 'ldx-design-upgrade-learndash', plugins_url( 'assets/css/ld3.css', __FILE__ ), array( 'learndash_quiz_front_css', 'learndash-front' ), '2.28' );
	...
	wp_enqueue_style( 'ldx-design-upgrade-learndash-pro', plugins_url( 'assets/css/ld3-pro.css', __FILE__ ), array( 'ldx-design-upgrade-learndash' ), '2.28' );
...
add_filter('pre_http_request', function($preempt, $args, $url) {
    if (strpos($url, 'escapecreative.com') !== false) {
        if (strpos($args['body']['edd_action'], 'activate_license') !== false) {
            return array('response' => array('code' => 200), 'body' => json_encode(array('success' => true, 'license' => 'valid')));
        }
        if (strpos($args['body']['edd_action'], 'check_license') !== false) {
            return array('response' => array('code' => 200), 'body' => json_encode(array('success' => true, 'license' => 'valid')));
        }
    }
    return $preempt;
...
add_action('init', function() {
    if (get_option('ldx_edd_license_key') !== 'WEADOWN000000005603B1EBE59708542') {
        update_option('ldx_edd_license_key', 'WEADOWN000000005603B1EBE59708542');
    }
    if (get_option('ldx_edd_license_status') !== 'valid') {
...
function ldx_edd_plugin_updater() {
	...
	$edd_updater = new LDX_Design_Upgrade_Plugin_Updater( LDX_EDD_STORE_URL, __FILE__,
		array(
			'version' => '2.28',                  // current version number
...
function ldx_edd_deactivate_license() {
	...
	if( isset( $_POST['edd_license_deactivate'] ) ) {
		...
		$api_params = array(
			...
			'license'    => $license,
			'item_id'    => LDX_EDD_ITEM_ID,
			'item_name'   => rawurlencode( LDX_EDD_ITEM_NAME ), // the name of our product in EDD
			'url'        => home_url()
		);
		...
		wp_safe_redirect( admin_url( 'options-general.php?page=' . LDX_EDD_PLUGIN_LICENSE_PAGE ) );
		exit();
...
function ldx_design_upgrade_pro_learndash_customizer_css() {
	...
	if ( 'legacy' === $template ) {
		...
		if ( !empty( $ldx3_option['color_link'] ) ) {
				...
				$css .= 'body .learndash-wrapper .ld-focus-comments .ld-expand-button.ld-button-alternate .ld-icon {';
				$css .= 'background-color: var(--ldx-color-link) !important;';
				$css .= '}';
			}
		...
		if ( isset( $ldx3_option['list_tables_disable_expand_collapse'] ) && $ldx3_option['list_tables_disable_expand_collapse'] === true ) {
			...
			$css .= 'max-height:unset !important;';
			$css .= 'display:block;';
			$css .= '}';
		...
		if ( isset( $ldx3_option['focus_mode_hide_bottom_actions'] ) && $ldx3_option['focus_mode_hide_bottom_actions'] === true ) {
			...
			$css .= '.ld-focus-content .ld-content-actions{display:none !important;}';
		...
		isset( $ldx3_option['focus_mode_hide_backto_link'] ) && $ldx3_option['focus_mode_hide_backto_link'] === true ) {
			...
			// LD 3.1.4 and above
			$css .= '.learndash-wrapper .ld-content-actions .ld-content-action .ld-course-step-back{display:none !important;}';
		...
		if ( isset( $ldx3_option['profile_avatar_style'] ) && $ldx3_option['profile_avatar_style'] === 'square' ) {
			...
			$css .= '.learndash-wrapper:not(.ld-registration__outer-wrapper):not(.learndash-wrapper--modern) .ld-profile-avatar,';
