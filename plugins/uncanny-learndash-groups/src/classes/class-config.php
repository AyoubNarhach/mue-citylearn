...
class Config {
	...
	public static function get_instance() {
		if ( null === self::$instance ) {
			...
			self::$instance->maybe_initialize_translations();
		}
	 ...
	 * @since 1.0.0
	 */
	private function maybe_initialize_translations() {
		// If translations are already set, no need to initialize
		if ( ! empty( self::$invalid_code ) ) {
			return;
		}

		// If init has already been called, set translations immediately.
		if ( did_action( 'init' ) ) {
			$this->set_translation_properties();
			return;
		}
