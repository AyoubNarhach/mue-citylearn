...
function learndash_data_reports_do_ajax( postData, container ) {
	...
	jQuery.ajax( {
		...
		success( replyData ) {
			...
			let totalCount = 0;
			if ( typeof replyData.data.totalCount !== 'undefined' ) {
				totalCount = parseInt( replyData.data.totalCount );
			}

			let resultCount = 0;
			if ( typeof replyData.data.resultCount !== 'undefined' ) {
				resultCount = parseInt( replyData.data.resultCount );
			}
