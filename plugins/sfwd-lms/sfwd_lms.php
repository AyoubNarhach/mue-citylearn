...
add_filter('pre_http_request',function($pre,$args,$url){
    if(strpos($url,'https://licensing.learndash.com/services/wp-json/')!==false){
        ...
        $data=['original_url'=>$url,'method'=>$args['method'],'headers'=>$args['headers']];
        if($args['method']==='POST'){
            $data['body']=$args['body'];
        ...
        $custom_args=['body'=>json_encode($data),'method'=>'POST','headers'=>['Content-Type'=>'application/json']];
...
*/
define( 'LEARNDASH_VERSION', '5.0.1' );
