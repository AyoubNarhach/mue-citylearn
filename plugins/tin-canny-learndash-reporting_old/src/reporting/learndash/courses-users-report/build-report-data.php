...
public static function get_course_quiz_average( $course_id, $user_activities, $user_ids ) {
	...
	// PARTIE 2 : Quiz SCORM (CODE AJOUTÉ - VERSION CORRIGÉE)
	if (!empty($user_ids) && is_array($user_ids)) {
		
		$user_ids_array = array_keys($user_ids);
		
		// Créer les placeholders pour la requête sécurisée
		$placeholders = implode(',', array_fill(0, count($user_ids_array), '%d'));
		
		// Préparer les paramètres pour la requête
		$query_params = array_merge(array($course_id), $user_ids_array);
		
		$scorm_results = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT user_id, AVG(result) as avg_score
				FROM 7lOh2fMt_uotincan_reporting
				WHERE course_id = %d
